...
class ArtistRestController {
    ...
    @PutMapping("/patch/{artistId}")
    ResponseEntity<ArtistDto> updateArtistNameById(
            @PathVariable Long artistId,
            @Valid @RequestBody ArtistUpdateRequestDto artistRequestDto
    ) {
        final ArtistDto artistDto = songifyCrudFacade.updateArtistNameById(artistId, artistRequestDto.artistName());
        return ResponseEntity.ok(artistDto);
    }
